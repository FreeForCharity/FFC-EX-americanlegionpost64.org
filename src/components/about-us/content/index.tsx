import React from 'react'

const Page = () => {
  return (
    <div className="pt-[40px] md:pt-[60px] pb-[20px] md:pb-[28px] bg-[#fcfcfc]">
      <div className="w-[95%] md:w-[85%] mx-auto pt-0 md:pt-[30px] px-0 md:px-[31px] pb-0 md:pb-[19px]">
        <h1 className="w-[95%] md:w-[80%] mx-auto text-[26px] md:text-[34px] font-[700] leading-[34px] md:leading-[37px] text-center text-[#f27022]">
          Reduce costs and increase revenues for nonprofits; putting that money back into their
          charitable mission where it belongs.
        </h1>

        <p className="text-[18px] md:text-[20px] font-[500] leading-[28px] md:leading-[30px] mt-[30px] md:mt-[50px] text-center w-[95%] md:w-[85%] mx-auto lato-font">
          This charity for charities seeks to replace as many functions as possible&nbsp;that
          current nonprofits pay for&nbsp;to for-profit companies with free or at-cost work from our
          campus,&nbsp;on-site projects,&nbsp;or partnerships with other entities.
        </p>
      </div>
    </div>
  )
}

export default Page
